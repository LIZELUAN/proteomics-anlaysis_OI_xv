...
da_filtered <- da %>% 
  ...
  filter(Num_valid_Ctrl>=2 & Num_valid_OI>=2) # # Select protein subset that capture the detected values#
...
# ============ PCA analysis ================
##################### Draw PCA based on DEP(statistical) ######################
...
pdf("DEG results/PCA.pdf",w=3,h=3)
...
pdf("DEG results/PCA_SVM_polynomial.pdf",w=4,h=4)
