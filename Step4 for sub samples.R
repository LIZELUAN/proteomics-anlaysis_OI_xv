...
da_filtered <- da %>% 
  ...
  filter(Num_valid_pro_Ctrl>=3 & Num_valid_pro_OI>=3)# # Select protein subset that capture the detected values#
...
data_normalized <- da_filtered %>%
  select(starts_with("Control") | starts_with("OI")) %>%
  select(-Control1,-OI1) %>%          # A little change
...
pdf("step4_results/PCA.pdf",w=4,h=4)
...
# Read in the data
dat <- data.frame(pca$x[,c(1,2)],Group=c(-2,-2,-2,2,2,2))
...
pdf("step4_results/PCA_SVM_3vs3.pdf",w=5,h=5)
plot(dat[,-3], cex=2,col= ifelse(dat$Group == -2, "lightblue","red"),pch = 19,
     main = 'SVM after Principal Component Analysis',
     xlab(paste("PC1 - ", pca.var.per[1], "%", sep = "")) ,
       ylab(paste("PC2 - ", pca.var.per[2], "%", sep = "")))
contour(X1, X2, matrix(as.numeric(ygrid),20,20), add=TRUE,level=0,lwd=2)
contour(X1, X2, matrix(as.numeric(ygrid), 20,20), add=TRUE,level=1,lwd=2,lty=2,col="red")
contour(X1, X2, matrix(as.numeric(ygrid), 20,20), add=TRUE,level=-1,lwd=2,lty=2,col="blue")
